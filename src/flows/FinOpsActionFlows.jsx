...
const FLOWS = {
  approve: {
    ...
    role: "FinOps Administrator only",
  ...
  hold: {
    ...
    steps: [
      { actor: "FinOps Administrator", action: "Navigate to payout detail", detail: "Click payout row from fleet or merchant payout list." },
      { actor: "FinOps Administrator", action: "Click \"Hold\" button", detail: "Opens hold dialog with warning: \"This payout will be placed on hold. No transfers will be initiated while on hold.\"" },
      { actor: "FinOps Administrator", action: "Select hold reason (required)", detail: "Options: Pending merchant verification, Suspicious activity review, Bank details under review, Regulatory hold, Internal audit, Other." },
      { actor: "FinOps Administrator", action: "Add internal note (optional)", detail: "Free text up to 300 characters. Context for the FinOps team — not visible to merchant." },
      { actor: "FinOps Administrator", action: "Confirm hold", detail: "Click \"Place hold\". 1s loading state." },
  ...
  abandon: {
    ...
    color: "#DC2626",
    bgColor: "#FEF2F2",
    precondition: "Payout is in Ready for Review status (POC restriction)",
    role: "FinOps Administrator only",
    transition: "Ready for Review → Abandoned (terminal). Hold flag cleared if present.",
    steps: [
      { actor: "FinOps Administrator", action: "Navigate to payout detail", detail: "Payout must be in Ready for Review status. POC restricts abandon to this status only." },
      { actor: "FinOps Administrator", action: "Click \"Abandon\" button", detail: "Opens destructive confirmation dialog with red-themed warning: \"This action is irreversible.\"" },
      { actor: "FinOps Administrator", action: "Review impact summary", detail: "Red panel shows: payout ID, merchant, amount at risk, transfers affected." },
      { actor: "FinOps Administrator", action: "Select reason (required)", detail: "Options: Duplicate payout, Merchant account closed, Fraudulent activity confirmed, Incorrect settlement calculation, Merchant requested cancellation, Other." },
      { actor: "FinOps Administrator", action: "Type \"ABANDON\" to confirm", detail: "Destructive action safeguard — must type exact text. Case-insensitive." },
      { actor: "FinOps Administrator", action: "Confirm abandon", detail: "Click \"Abandon payout\". 1.5s loading state (longer delay for gravity of action)." },
    ...
    edgeCases: [
      "POC: abandon restricted to Ready for Review only — conservative approach to minimise risk of internal system discrepancies",
      "Held payouts can be abandoned if the underlying status is Ready for Review",
      ...
      "Future consideration: expand abandon to other statuses if real scenarios are discovered that require it",
    ],
  },
  execute: {
    ...
    role: "FinOps Administrator only (POC: manual, Pilot/BAU: automated)",
    transition: "Ready for Transfer → Transferring → Completed / Failed",
    steps: [
      { actor: "FinOps Administrator", action: "Navigate to approved payout", detail: "Payout must be in Ready for Transfer status and not on hold." },
      { actor: "FinOps Administrator", action: "Click \"Begin transfer\" button", detail: "Triggers transfer immediately — no additional confirmation dialog for speed." },
  ...
  retry: {
    ...
    role: "FinOps Administrator only (POC: manual only. Pilot/BAU: automated for select conditions)",
  ...
  release: {
    ...
    steps: [
      { actor: "FinOps Administrator", action: "Navigate to held payout", detail: "Filter by On Hold to find held payouts." },
      { actor: "FinOps Administrator", action: "Click \"Release Hold\" button", detail: "Immediate action — no confirmation dialog needed." },
