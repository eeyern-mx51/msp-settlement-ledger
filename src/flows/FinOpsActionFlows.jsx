...
const FLOWS = {
  approve: {
    ...
    steps: [
      { actor: "Toby", action: "Navigate to payout detail", detail: "Click payout row from fleet or merchant payout list." },
      { actor: "Toby", action: "Review payout summary", detail: "Validate payout amount against expected DTE transaction sums. Check merchant, MID, transfer count." },
      { actor: "Toby", action: "Click \"Approve\" button", detail: "Opens confirmation dialog showing payout ID, merchant, MID, amount, and transfer count." },
      { actor: "Toby", action: "Confirm approval", detail: "Click \"Approve payout\" in dialog. 1.2s loading state while processing." },
  ...
  hold: {
    ...
    steps: [
      { actor: "Toby", action: "Navigate to payout detail", detail: "Click payout row from fleet or merchant payout list." },
      { actor: "Toby", action: "Click \"Hold\" button", detail: "Opens hold dialog with warning: \"This payout will be placed on hold. No transfers will be initiated while on hold.\"" },
      { actor: "Toby", action: "Select hold reason (required)", detail: "Options: Pending merchant verification, Suspicious activity review, Bank details under review, Regulatory hold, Internal audit, Other." },
      { actor: "Toby", action: "Add internal note (optional)", detail: "Free text up to 300 characters. Context for the FinOps team — not visible to merchant." },
      { actor: "Toby", action: "Confirm hold", detail: "Click \"Place hold\". 1s loading state." },
  ...
  abandon: {
    ...
    steps: [
      { actor: "Toby", action: "Navigate to payout detail", detail: "Payout must be in Ready for Review, Ready for Transfer, or Failed status." },
      { actor: "Toby", action: "Click \"Abandon\" button", detail: "Opens destructive confirmation dialog with red-themed warning: \"This action is irreversible.\"" },
      { actor: "Toby", action: "Review impact summary", detail: "Red panel shows: payout ID, merchant, amount at risk, transfers affected." },
      { actor: "Toby", action: "Select reason (required)", detail: "Options: Duplicate payout, Merchant account closed, Fraudulent activity confirmed, Incorrect settlement calculation, Merchant requested cancellation, Other." },
      { actor: "Toby", action: "Type \"ABANDON\" to confirm", detail: "Destructive action safeguard — must type exact text. Case-insensitive." },
      { actor: "Toby", action: "Confirm abandon", detail: "Click \"Abandon payout\". 1.5s loading state (longer delay for gravity of action)." },
  ...
  execute: {
    ...
    role: "Toby only (POC: manual, Pilot/BAU: automated)",
    transition: "Ready for Transfer → Transferring → Completed / Failed",
    steps: [
      { actor: "Toby", action: "Navigate to approved payout", detail: "Payout must be in Ready for Transfer status and not on hold." },
      { actor: "Toby", action: "Click \"Begin transfer\" button", detail: "Triggers transfer immediately — no additional confirmation dialog for speed." },
  ...
  retry: {
    ...
    role: "Toby only (POC: manual only. Pilot/BAU: automated for select conditions)",
    transition: "Failed (retryable) → Ready for Transfer → Begin transfer → Transferring → Completed / Failed",
    steps: [
      { actor: "Toby", action: "Navigate to failed payout", detail: "Filter by Failed status. Review the error code and failure reason displayed in the alert banner." },
      { actor: "Toby", action: "Review failure context", detail: "Check error code (e.g. GATEWAY_TIMEOUT, RATE_LIMITED), attempt count, and recommended action. For GATEWAY_TIMEOUT, confirm no duplicate payment was processed." },
      { actor: "Toby", action: "Click \"Retry (attempt N)\" button", detail: "Button shows the attempt number. Available only on retryable failures. Triggers immediately — no confirmation dialog." },
      ...
      { actor: "Toby", action: "Begin the transfer", detail: "Payout now follows the standard Begin transfer flow — click \"Begin transfer\" to initiate the NPP transfer." },
  ...
  release: {
    ...
    steps: [
      { actor: "Toby", action: "Navigate to held payout", detail: "Filter by On Hold to find held payouts." },
      { actor: "Toby", action: "Click \"Release Hold\" button", detail: "Immediate action — no confirmation dialog needed." },
