...
const FLOWS = {
  ...
  execute: {
    ...
    steps: [
      ...
      { actor: "FinOps Admin", action: "Click \"Execute transfer\" button", detail: "Triggers transfer immediately — no additional confirmation dialog for speed." },
      { actor: "System", action: "Set status to Transferring", detail: "Status MUST be set before the NPP request is made. Audit log: \"Execute transfer\"." },
    ...
    edgeCases: [
      "Fleet or merchant-level hold blocks execution entirely",
      "Payouts on hold cannot have transfer executed — hold must be released first",
  ...
  retry: {
    ...
    transition: "Failed (retryable) → Ready for Transfer → Execute transfer → Transferring → Completed / Failed",
    steps: [
      ...
      { actor: "FinOps Admin", action: "Execute the transfer", detail: "Payout now follows the standard Execute transfer flow — click \"Execute transfer\" to initiate the NPP transfer." },
    ...
    edgeCases: [
      ...
      "Non-retryable resolution flow: fix root cause → FinOps transitions payout to Ready for Transfer → Execute transfer (this is a fresh attempt, not a retry of old data)",
