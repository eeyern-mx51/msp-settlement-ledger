    ...
    items: [
      {
        ...
        uiTreatment: "Indigo badge. Primary action: Approve. Secondary actions: Hold, Abandon. Abandon is only available from this status.",
        useCases: [
      ...
      },
      {
        ...
        uiTreatment: "Green badge. Primary action: Begin transfer. Secondary action: Hold.",
        useCases: [
      ...
      },
      {
        ...
        uiTreatment: "Red badge. Additional sub-badge: 'Retryable' (purple) or 'Non-retryable' (grey). Retryable payouts show a Retry action button. Non-retryable payouts show a recommended resolution action.",
        useCases: [
      ...
      },
      {
        ...
        color: "#9CA3AF",
        bg: "#F9FAFB",
        definition: "Terminal state. The payout has been permanently cancelled. The merchant's ledger entries are released back to the balance for inclusion in a future payout. This action is irreversible. POC: abandon is only available from Ready for Review status.",
    ...
    items: [
      {
        ...
        color: "#FBBF24",
        bg: "#FFFBEB",
        definition: "A hold placed on a payout to temporarily freeze all progression. The underlying status (Ready for Review or Ready for Transfer) is preserved. While on hold, no status transitions occur except Release Hold. Abandon is only available if the underlying status is Ready for Review. Holds can be placed at three scopes: fleet level (\"Hold all payouts\"), merchant level (\"Hold payouts for [Merchant Name]\"), or individual payout level (\"Place on hold\").",
        uiTreatment: "Amber 'On Hold' badge displayed alongside the status badge. Actions change to: Release Hold (primary). Abandon shown only if underlying status is Ready for Review. The underlying status badge remains visible so FinOps always knows where the payout was when the hold was placed. Merchant-level holds use the actual merchant name in the button, dialog, and banner to disambiguate from fleet-level holds.",
        useCases: [
      ...
      },
      {
        ...
        uiTreatment: "Sub-badge next to the Failed badge: purple 'Retryable' or grey 'Non-retryable'. Retryable payouts show a Retry button. Non-retryable payouts show a recommended resolution action.",
        useCases: [
    ...
    items: [
      ...
      },
      {
        ...
        rules: [
          ...
          { rule: "No abandon from Failed (POC)", detail: "POC restricts abandon to Ready for Review only. Failed payouts must be resolved through retry or manual resolution. This conservative approach avoids internal system discrepancies while we discover whether abandoning from other statuses is genuinely needed.", futureAutomation: false },
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        color: "#6B7280",
        bg: "#F9FAFB",
        definition: "Permanently cancels a payout. POC: only available from Ready for Review status. Requires typing 'ABANDON' to confirm.",
        uiTreatment: "Red outline button (shown only on Ready for Review payouts). Confirmation dialog is deliberately high-friction: shows at-risk amounts, requires reason selection, and demands typing 'ABANDON' in uppercase.",
        auditExamples: [
        ...
        uxJustification: "See the Abandoned status entry above. The high-friction confirmation pattern (type-to-confirm) is standard for irreversible financial actions. POC restricts abandon to Ready for Review only — the conservative approach minimises risk while we learn whether other statuses genuinely need it.",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        color: "#4F46E5",
        bg: "#EEF2FF",
        definition: "Full read-write access to all payout operations. Can Approve, Begin Transfer, Hold, Release Hold, Retry, and Abandon payouts (abandon restricted to Ready for Review in POC). Can place fleet-level holds (\"Hold all payouts\") and merchant-level holds (\"Hold payouts for [Merchant Name]\").",
