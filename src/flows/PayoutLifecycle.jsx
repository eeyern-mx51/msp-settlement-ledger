...
const STATES = [
  { id: "ready_for_review", label: "Ready for\nReview", x: 350, y: 60, color: "#EEF2FF", border: "#818CF8", textColor: "#4338CA", description: "Starting state for a payout where mx51 owes money to the merchant. Can be paused (flag overlay).", entry: "Payout is prepared (automated daily job sweeps merchant balance to zero).", exits: ["Approved → Ready for Transfer", "Pause → Paused flag set (stays in this state)", "Abandon → Abandoned", "Zero/negative balance → auto-Completed"] },
  { id: "ready_for_transfer", label: "Ready for\nTransfer", x: 350, y: 220, color: "#F0FDF4", border: "#86EFAC", textColor: "#166534", description: "Payment can be executed. Indicates the earliest execution time has been reached. Can be paused (flag overlay).", entry: "FinOps approves the payout after review, or retryable failure auto-transitions here.", exits: ["Execute → Transferring", "Pause → Paused flag set (stays in this state)", "Abandon → Abandoned"] },
  { id: "transferring", label: "Transferring", x: 350, y: 380, color: "#F5F3FF", border: "#C084FC", textColor: "#7C3AED", description: "Payout is being transferred to the merchant via NPP. Status MUST be set before the NPP request. Cannot be paused or abandoned.", entry: "FinOps clicks Execute (POC: manual, Pilot/BAU: automated).", exits: ["Success → Completed", "Failure (retryable) → Ready for Transfer", "Failure (non-retryable) → Failed"] },
...
const NOTE = {
  id: "paused_note", label: "Paused (flag)", x: 80, y: 60, color: "#FFFBEB", border: "#FBBF24", textColor: "#92400E",
  description: "Paused is a boolean flag overlay, not a separate state. Only payouts in Ready for Review or Ready for Transfer can be paused. While paused, the underlying status is preserved. Resume clears the flag. Paused payouts can also be abandoned.",
};
  ...
  return (
    <div className="min-h-screen bg-white p-8 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-6 inline-block">Updated: Paused is now a flag overlay (not a separate state). Abandon allowed directly from Review, Transfer, and Failed.</p>
        ...
        <div className="flex gap-4 mb-6 text-xs">
          ...
          <div className="flex items-center gap-1.5"><div className="w-6 h-5 rounded border-2 border-dashed border-amber-400 bg-amber-50" /><span className="text-gray-600">Paused flag (overlay)</span></div>
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden bg-gray-50">
          <svg viewBox="0 0 800 640" className="w-full" style={{ maxHeight: 540 }}>
            ...
            {/* Pause flag indicator arrows - dashed amber from Review and Transfer to Note */}
            ...
            <text x={270} y={68} fontSize="8" fill="#D97706" fontWeight="600" fontFamily="system-ui">Pause</text>
            ...
            <text x={268} y={168} fontSize="8" fill="#D97706" fontWeight="600" fontFamily="system-ui">Pause</text>
        ...
        <div className="mt-6 bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
            ...
            <p className="text-xs text-gray-500 mt-0.5">12 transitions reflecting the updated lifecycle with Paused as flag overlay</p>
          </div>
          ...
          <div className="px-6 py-3 bg-amber-50 border-t border-amber-200">
            <p className="text-xs text-amber-800"><strong>Paused flag:</strong> Can be set on Ready for Review or Ready for Transfer. While paused, no transitions occur except Resume (clears flag) and Abandon. Paused is not a state — it's an overlay on the existing status.</p>
          </div>
        </div>
        ...
        {selected && (
          <div className="mt-6 bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              {selected.id === "paused_note" && <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-amber-50 text-amber-700">Flag overlay</span>}
            </div>
